						...
						CBHashLK *ptrHashObjs/*=NULL*/ )
{
	...
	m_ptrHashObjs = NULL; 
	SetFromResID(hMenuParent, idResMenuItem, hWndParent, ptrHashMenuIDs, ptrHashObjs);
}
...
void CBMenuItem::SetFromResID( HMENU hMenuParent, UINT idResMenuItem, HWND hWndParent, CBHashLK *ptrHashMenuIDs, CBHashLK *ptrHashObjs )
{
...
void CBMenuItem::SetFromPos( HMENU hMenuParent, UINT pos, HWND hWndParent, CBHashLK *ptrHashMenuIDs, CBHashLK *ptrHashObjs )
{
	m_hMenuParent = hMenuParent;
...
void CBMenuItem::BitmapSet( UINT idResBmp )
{
	...
	if (hBmp)
	{
		m_ptrHashObjs->Add((long)hBmp, (long)hBmp, IMAGE_BITMAP, 0, 0, 0, 0, false);
		BitmapSet(hBmp);
	}
}
...
BOOL CBMenuItem::SetCheckBitmaps( UINT idResBmpUnChecked, UINT idResBmpChecked )
{
	...
	// 位图句柄存入 m_ptrHashObjs，以在窗体卸载时自动 DeleteObject
	m_ptrHashObjs -> Add((long)hBmpUnCheck, (long)hBmpUnCheck, IMAGE_BITMAP, 0, 0, 0, 0, false);
	m_ptrHashObjs -> Add((long)hBMPCheck, (long)hBMPCheck, IMAGE_BITMAP, 0, 0, 0, 0, false);
