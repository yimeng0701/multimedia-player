...
public:
	// 构造函数
	CBMenuItem(HMENU hMenuParent=NULL, UINT idResMenuItem=0, HWND hWndParent=NULL, CBHashLK * ptrHashMenuIDs=NULL, CBHashLK * m_ptrHashObjs=NULL);	
	// 析构函数
	~CBMenuItem();
	...
	void SetFromResID(HMENU hMenuParent, UINT idResMenuItem, HWND hWndParent, CBHashLK *ptrHashMenuIDs, CBHashLK *ptrHashObjs );
	...
	// ptrHashObjs 为菜单项隶属 Form 对象（CBForm类）中维护的位图、光标、图标等对象的
	//   句柄哈希表的地址，用于加载菜单相关位图等资源时将句柄存入其中
	void SetFromPos(HMENU hMenuParent, UINT pos, HWND hWndParent, CBHashLK *ptrHashMenuIDs, CBHashLK *ptrHashObjs );
...
private:
	...
	CBHashLK * m_ptrHashObjs;	// 菜单项隶属 Form 对象（CBForm类）中维护的位图、光标、图标等对象的句柄哈希表的地址，在Form 对象析构时被自动释放，菜单项相关的资源加载（如位图）应将所加载的资源句柄存入其中
};
